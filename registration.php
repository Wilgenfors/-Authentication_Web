...
if ($_SERVER["REQUEST_METHOD"] == "POST") { // Проверяем, был ли запрос методом POST
    ...
    if (empty($name) || empty($email) || empty($password)) {
        ...
        header("Location: /data_entered_incorrectly.php"); // Перенаправление на страницу с ошибкой
   ...
   // Проверка существования email
    $checkEmailQuery = "SELECT * FROM users WHERE email = '$email_sql'";
    $result = $conection->query($checkEmailQuery);

    if ($result->num_rows > 0) {
        // Если email уже существует
        $_SESSION['success_message'] = "Пользователь с таким email уже существует.";
        header("Location: /data_entered_incorrectly.php"); // Перенаправление на страницу с ошибкой
        exit; // Завершаем выполнение скрипта после перенаправления
    }
    ...
    header("Location: /succefuly_log_in.php"); // Перенаправление на страницу успешной регистрации
