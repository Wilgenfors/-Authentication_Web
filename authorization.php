...
if ($_SERVER["REQUEST_METHOD"] == "POST") { // Проверяем, был ли отправлен POST-запрос
    ...
    if (empty($email) || empty($password)) {
        ...
        header("Location: /registration/profily_incorrectly.php"); // Перенаправляем на страницу с ошибкой
    ...
    if (mysqli_num_rows($check_user) > 0) { // Если пользователь найден
        ...
        if (password_verify($password_sql, $user['password'])) { // Если пароль верный
            ...
            header("Location: /registration/profily_succefuly.php"); // Перенаправляем на страницу профиля
            exit(); // Завершаем выполнение скрипта
        } else {
            // Неправильный пароль
